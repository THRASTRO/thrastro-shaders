...
(function(h,e){function f(k,n,t,u){t=t.shader||"";u=u?1:0;Array.isArray(t)&&(t=t.join("\n"));return k.splice(n,u,t)}function c(k,n){if(k&&k.length)for(var t=0;t<k.length;t++){for(var u=!1,l=k[t],p=0;p<n.length;p++)l.after&&l.after.test(n[p])?(f(n,p+1,l,!1),p+=2,u=!0):l.before&&l.before.test(n[p])?(f(n,p,l,!1),p+=2,u=!0):l.replace&&l.replace.test(n[p])&&(f(n,p,l,!0),p+=1,u=!0);if(!u)throw Error("Shader Chunk not found");}}function a(k,n){k=h.ShaderChunk[n];if(void 0===k)throw Error("Can not resolve #include <"+
...
return u}class d extends h.RawShaderMaterial{constructor(k){k=k||{};k.glslVersion="300 es";super(k)}}class g extends h.Mesh{constructor(k,n){super(k,n);n.pbody=this}}const m=/^[ \t]*#include +<([\w\d./]+)>/gm;h.ShaderLib.none={uniforms:{},vertexShader:"",fragmentShader:""};e.CustomMaterial=b;class q extends b(h.ShaderMaterial,"CustomShaderMaterial","basic"){constructor(k){super(k)}}class x extends b(h.MeshBasicMaterial,"CustomMeshBasicMaterial","basic"){constructor(k){super(k)}}class y extends b(h.MeshLambertMaterial,
"CustomMeshLambertMaterial","lambert"){constructor(k){super(k)}}class z extends b(h.MeshPhongMaterial,"CustomMeshPhongMaterial","phong"){constructor(k){super(k)}}class A extends b(h.MeshPhysicalMaterial,"CustomMeshPhysicalMaterial","physical"){constructor(k){super(k)}}e.Mesh=g;e.CustomRawShader=d;e.CustomShaderMaterial=q;e.CustomMeshBasicMaterial=x;e.CustomMeshLambertMaterial=y;e.CustomMeshPhongMaterial=z;e.CustomMeshPhysicalMaterial=A;e.StartStaticForLoop=function(k,n){return[`#if ${n} > 0`,`#if ${n} > 1`,
`  for ( int ${k} = 0; ${k} < ${n}; ++ ${k} ) {`,"#else",`  { int ${k} = 0;`,"#endif\n"].join("\n")};e.EndStaticForLoop=function(k,n){return"  }\n#endif\n"}})(THREE,THRAPP);window.THRAPP||(window.THRAPP={});
(function(h,e){class f extends e.CustomRawShader{constructor(c){c=c||{};super(c);this.isRawShaderMaterial=!1;this.uniforms.time={type:"f",value:c.time||0};this.uniforms.scale={type:"f",value:c.scale||1};this.uniforms.opacity={type:"f",value:c.opacity||1};this.uniforms.camFadeDist={type:"f",value:c.camFadeDist||5E9};this.uniforms.color={type:"v3",value:new h.Color(c.color||16777215)};this.vertexShader="#include <common>\n#include <logdepthbuf_pars_vertex>\nuniform float time;\nuniform float scale;\nuniform float opacity;\nuniform float camFadeDist;\nuniform vec3 color;\nattribute vec2 attributes;\nvarying vec4 fragColor;\nvoid main() {\n float ra = position.x;\n float dec = position.y;\n float dist = position.z;\n float pm_ra = attributes.x;\n float pm_dec = attributes.y;\n ra += pm_ra * time / 206300000.0;\n dec += pm_dec * time / 206300000.0;\n float x = cos(dec) * cos(ra) * dist * scale;\n float y = cos(dec) * sin(ra) * dist * scale;\n float z = sin(dec) * dist * scale;\n vec4 pos = vec4(x, y, z, 1.0);\n gl_Position = projectionMatrix * modelViewMatrix * pos;\n float camdist = length(cameraPosition.xyz);\n float alpha = 1.0 - smoothstep(0.0, camFadeDist, camdist);\nfragColor = vec4(color.xyz, alpha * opacity);\n#include <logdepthbuf_vertex>\n}";
this.fragmentShader="#include <common>\n#include <logdepthbuf_pars_fragment>\nout highp vec4 pc_fragColor;\n#define gl_FragColor pc_fragColor\nvarying vec4 fragColor;\nvoid main() {\n gl_FragColor = fragColor;\n #include <logdepthbuf_fragment>\n}";if(c.datgui){var a=c.datgui,b=this.uniforms;c.name&&(a=a.addFolder(c.name));a.add(b.scale,"value",0,9).step(.001).name("scale");a.add(b.opacity,"value",0,1).step(.001).name("opacity");a.add(b.camFadeDist,"value",0,5E6).step(.001).name("fade dist");a.addThreeColor(b.color,
...
this.vertexFilter=this.fragmentFilter=function(d){return d.replace(/attribute vec3 position;/g,"attribute vec4 position;").replace(/attribute vec[23] (?:normal|uv);/g,"")};if(c.datgui){var a=c.datgui,b=this.uniforms;c.name&&(a=a.addFolder(c.name));a.add(b.scale,"value",-24,64).step(.001).name("scale");a.add(b.minMag,"value",-24,64).step(.001).name("minMag");a.add(b.maxMag,"value",-24,64).step(.001).name("maxMag");a.add(b.opacity,"value",0,1).step(.001).name("opacity");a.add(b.magFact,"value",0,80).step(.001).name("magFact");
a.add(b.magScale,"value",0,5).step(.001).name("magScale");a.add(b.sizeScale,"value",0,40).step(.001).name("sizeScale");a.add(b.fillFactor,"value",.5,2).step(.001).name("fillFactor");a.add(b.fillPower,"value",.25,4).step(.001).name("fillPower");a.add(b.falloff,"value",0,5E5).step(.001).name("falloff");a.add(b.dimming,"value",0,4).step(.001).name("dimming")}}addUniforms(c){super.addUniforms(c)}updateUniforms(){super.updateUniforms()}}f.prototype.time=null;f.prototype.fov=null;f.prototype.scale=null;
...
if(c.datgui){var a=c.datgui,b=this.uniforms;c.name&&(a=a.addFolder(c.name));a.add(b.lucency,"value",0,1).step(.001).name("lucency");a.add(b.attenuate,"value",0,9).step(.001).name("attenuate");a.add(b.trailStart,"value",0,1).step(.001).name("trailStart");a.add(b.trailLength,"value",-1,1).step(.001).name("trailLength")}}addOrbital(c){}addUniforms(c){super.addUniforms(c)}updateUniforms(){super.updateUniforms()}getVertexChunks(){var c=super.getVertexChunks();c.push({after:/<common>/,shader:[]});c.push({before:/<begin_vertex>/,
shader:[]});return c}getFragmentChunks(){var c=super.getFragmentChunks();c.push({after:/<common>/,shader:[]});c.push({before:/<aomap_fragment>/,shader:[]});c.push({after:/<aomap_fragment>/,shader:[]});c.push({after:/vec3 outgoingLight/,shader:[]});return c}}f.prototype.lucency=null;f.prototype.attenuate=null;f.prototype.trailLength=null;f.prototype.trailStart=null;f.prototype.trailEnd=null;e.OrbitalsShader=f})(THREE,THRAPP);window.THRAPP||(window.THRAPP={});
THRAPP.BaseMaterial=THRAPP.BaseMaterial||THRAPP.CustomMeshPhysicalMaterial;
(function(h,e){var f=new h.Matrix4;class c extends e.BaseMaterial{constructor(a){super(a);this.setDefines({NUM_STARS:0,NUM_ECLIPSERS:0});this.stars=[];this.eclipsers=[];this.addStar(a.stars||[]);this.addEclipser(a.eclipsers||[]);this.uniforms.bodyRadius.value=a.radius||0;null!=a.laserSize&&(this.defines.USE_ECLIPSE_LASER="",this.uniforms.laserSize.value=a.laserSize);this.pbody=a.pbody||null}addStar(a){Array.isArray(a)||(a=[a]);for(var b=0;b<a.length;b++)this.stars.push(a[b]),this.uniforms.lightPos.value.push(new h.Vector3),
...
a.eclipserSize={type:"fv1",value:[]};a.eclipserPos={type:"v3v",value:[]};a.lightSize={type:"fv1",value:[]};a.lightPos={type:"v3v",value:[]};a.laserSize={type:"f",value:5E-7};a.bodyRadius={type:"f",value:5E-7}}updateUniforms(){var a=this.uniforms,b=this.eclipsers,d=this.stars;super.updateUniforms();this.pbody&&(this.pbody.updateMatrix(),f.copy(this.pbody.matrixWorld).invert());for(var g=0;g<b.length;g+=1)a.eclipserPos.value[g].setFromMatrixPosition(b[g].matrixWorld),a.eclipserPos.value[g].applyMatrix4(f);
for(g=0;g<d.length;g+=1)a.lightPos.value[g].setFromMatrixPosition(d[g].matrixWorld),a.lightPos.value[g].applyMatrix4(f)}debug(a,b){var d=new h.Vector3,g=this.uniforms.lightPos.value,m=this.uniforms.lightSize.value,q=this.uniforms.eclipserPos.value,x=this.uniforms.eclipserSize.value,y=this.uniforms.bodyRadius.value;console.log("Body Radius: ",y);console.log("Light[",a,"] - size: ",m[a],", pos: ",g[a]);console.log("Eclipser[",b,"] - size: ",x[b],", pos: ",q[b]);d=d.copy(g[a]).sub(q[b]).length();q=q[b].length()-
...
(function(h,e){var f=new h.Matrix4;class c extends e.PlanetShader{constructor(a){super(a);this.setDefines({USE_GROUNDSHADER:1,NUM_SAMPLES:16});if(!a.camera)throw"Camera missing";this.camera=a.camera;if(a.atmosphere){a=a.atmosphere;var b=this.uniforms;a.G=a.G||-.95;a.Kr=a.Kr||.0025;a.Km=a.Km||.001;a.ESun=a.ESun||20;a.bias=a.bias||.5;a.skale=a.skale||.5;a.exposure=a.exposure||2.5;a.scaleDepth=a.scaleDepth||.25;a.scaleHeight=a.scaleHeight||1.025;a.innerRadius=a.innerRadius||0;var d=a.wavelength;b.v3InvWavelength.value.set(1/
Math.pow(d.x,4),1/Math.pow(d.y,4),1/Math.pow(d.z,4));d=a.G;b.fg.value=d;b.fg2.value=d*d;d=a.Kr;var g=a.Km,m=a.ESun;b.fKrESun.value=d*m;b.fKmESun.value=g*m;b.fKr4PI.value=4*d*Math.PI;b.fKm4PI.value=4*g*Math.PI;d=a.scaleDepth;g=a.innerRadius;m=g+a.height;b.fBias.value=a.bias;b.fExposure.value=a.exposure;b.fScale.value=1/(m-g);b.fScaleDepth.value=d;b.fOuterRadius.value=m;b.fInnerRadius.value=g;b.fOuterRadius2.value=m*m;b.fInnerRadius2.value=g*g}}updateUniforms(){var a=this.uniforms;super.updateUniforms();
var b=a.fCameraPos.value;b.setFromMatrixPosition(camera.matrixWorld);this.pbody&&this.pbody.matrixWorld&&(f.copy(this.pbody.matrixWorld).invert(),b.applyMatrix4(f));b=b.lengthSq();a.fCameraHeight.value=Math.sqrt(b);a.fCameraHeight2.value=b}addUniforms(a){super.addUniforms(a);a.fg={type:"f",value:0};a.fg2={type:"f",value:0};a.fKrESun={type:"f",value:0};a.fKmESun={type:"f",value:0};a.fKr4PI={type:"f",value:0};a.fKm4PI={type:"f",value:0};a.fScale={type:"f",value:0};a.fBias={type:"f",value:0};a.fExposure=
{type:"f",value:0};a.fScaleDepth={type:"f",value:0};a.fOuterRadius={type:"f",value:0};a.fInnerRadius={type:"f",value:0};a.fOuterRadius2={type:"f",value:0};a.fInnerRadius2={type:"f",value:0};a.fCameraHeight={type:"f",value:0};a.fCameraHeight2={type:"f",value:0};a.v3InvWavelength={type:"v3",value:new h.Vector3};a.fCameraPos={type:"v3",value:new h.Vector3}}getVertexChunks(){var a=super.getVertexChunks();a.push({after:/<common>/,shader:["#if USE_GROUNDSHADER > 0 && NUM_STARS > 0","  uniform vec3 lightPos[NUM_STARS];",
"  uniform float lightSize[NUM_STARS];","  uniform vec3 fCameraPos;       // The Camera Position","  uniform vec3 v3InvWavelength;   // 1 / pow(abs(wavelength), 4) for the red, green, and blue channels","  uniform float fCameraHeight;    // The cameras current height","  uniform float fCameraHeight2;   // fCameraHeight^2","  uniform float fOuterRadius;     // The outer (atmosphere) radius","  uniform float fOuterRadius2;    // fOuterRadius^2","  uniform float fInnerRadius;     // The inner (planetary) radius",
"  uniform float fInnerRadius2;    // fInnerRadius^2","  uniform float fKrESun;          // Kr * ESun","  uniform float fKmESun;          // Km * ESun","  uniform float fKr4PI;           // Kr * 4 * PI","  uniform float fKm4PI;           // Km * 4 * PI","  uniform float fScale;           // 1 / (fOuterRadius - fInnerRadius)","  uniform float fScaleDepth;      // The scale depth (i.e. the altitude at which the atmospheres average density is found)","  uniform float fBias;            // GroundShader Bias",
"  varying vec3 v3RayleighColor;","  varying vec3 v3MieColor;","  float scale(float fCos)","  {","  \tfloat x = 1.0 - fCos;","  \treturn fScaleDepth * exp(-0.00287 + x*(0.459 + x*(3.83 + x*(-6.80 + x*5.25))));","  }","#endif"]});a.push({after:/<begin_vertex>/,shader:["#if USE_GROUNDSHADER > 0 && NUM_STARS > 0","  v3RayleighColor *= 0.0;","  v3MieColor *= 0.0;","  float fInvScaleDepth = (1.0 / fScaleDepth);","  float fScaleOverScaleDepth = fScale / fScaleDepth;","  // Do all the calculation in local space, since this yields best precision",
"  vec3 cameraLocal = fCameraPos; // vec3(modelMatrixInverse * vec4(cameraPosition, 1.0));","  // Get the ray from the camera to the vertex and its length","  // which is the far point of the ray passing through the atmosphere","  vec3 v3Ray = position - cameraLocal;","  float fFar = length(v3Ray);","  v3Ray /= fFar;","  // Calculate the closest intersection of the ray with the outer atmosphere","  // Which is the near point of the ray passing through the atmosphere","  float B = 2.0 * dot(cameraLocal, v3Ray);",
"  float C = fCameraHeight2 - fOuterRadius2;","  float fDet = max(0.0, B*B - 4.0 * C);","  float fNear = 0.5 * (-B - sqrt(fDet));","  float fDepth = exp((fInnerRadius - fOuterRadius) * fScaleOverScaleDepth);","  float fCameraAngle = dot(-v3Ray, position) / length(position);","  float fCameraScale = scale(fCameraAngle);","  float fCameraOffset = fDepth*fCameraScale;",e.StartStaticForLoop("n","NUM_STARS"),"    // Do all the calculation in local space, since this yields best precision","    vec3 lightLocal = lightPos[n]; // vec3(modelMatrixInverse * vec4(starPos[n], 1.0));",
"    // Calculate the ray's starting position, then calculate its scattering offset","    vec3 v3Start = cameraLocal + v3Ray * fNear;","    fFar -= fNear;","    float fLightAngle = dot(lightLocal, position) / length(position);","    float fLightScale = scale(fLightAngle);","    float fTemp = (fLightScale + fCameraScale);","    // Initialize the scattering loop variables","    float fSampleLength = fFar / float(NUM_SAMPLES);","    float fScaledLength = fSampleLength * fScale;","    vec3 v3SampleRay = v3Ray * fSampleLength;",
"    vec3 v3SamplePoint = v3Start + v3SampleRay * 0.5;","    // Now loop through the sample rays","    vec3 v3FrontColor = vec3(0.0, 0.0, 0.0);","    vec3 v3Attenuate;","    for(int i=0; i<NUM_SAMPLES; i++)","    {","        float fHeight = length(v3SamplePoint);","        float fDepth = exp(fScaleOverScaleDepth * (fInnerRadius - fHeight));","        float fScatter = fDepth*fTemp - fCameraOffset;","        v3Attenuate = exp(-fScatter * (v3InvWavelength * fKr4PI + fKm4PI));","        v3FrontColor += v3Attenuate * (fDepth * fScaledLength);",
"        v3SamplePoint += v3SampleRay;","    }","    v3RayleighColor += v3FrontColor * (v3InvWavelength * fKrESun + fKmESun) / float(NUM_STARS);","    v3MieColor += v3Attenuate / float(NUM_STARS);",e.EndStaticForLoop("n","NUM_STARS"),"#endif"]});return a}getFragmentChunks(){var a=super.getFragmentChunks();a.push({after:/<common>/,shader:["#if USE_GROUNDSHADER > 0 && NUM_STARS > 0","  varying vec3 v3RayleighColor;","  varying vec3 v3MieColor;","  uniform float fBias;","#endif"]});a.push({after:/vec3 outgoingLight/,
shader:["#if USE_GROUNDSHADER > 0 && NUM_STARS > 0","  vec3 color = v3RayleighColor + 0.25 * v3MieColor;","  outgoingLight = mix(outgoingLight, color, fBias);","#endif"]});return a}}c.prototype.atmosphere=null;e.GroundShader=c})(THREE,THRAPP);window.THRAPP||(window.THRAPP={});
(function(h,e){var f=new h.Matrix4;class c extends e.CustomMaterial(h.ShaderMaterial,"ShaderMaterial","basic"){constructor(a){super(a);this.setDefines({NUM_STARS:0,USE_ATMOSPHERE:""});if(a.atmosphere){var b=a.atmosphere,d=this.uniforms;b.G=b.G||-.95;b.Kr=b.Kr||.0025;b.Km=b.Km||.001;b.ESun=b.ESun||20;b.bias=b.bias||.5;b.skale=b.skale||.5;b.exposure=b.exposure||2.5;b.scaleDepth=b.scaleDepth||.25;b.scaleHeight=b.scaleHeight||1.025;b.innerRadius=b.innerRadius||r*KM2AU;var g=b.wavelength;d.v3InvWavelength.value.set(1/
Math.pow(g.x,4),1/Math.pow(g.y,4),1/Math.pow(g.z,4));g=b.G;d.fg.value=g;d.fg2.value=g*g;g=b.Kr;var m=b.Km,q=b.ESun;d.fKrESun.value=g*q;d.fKmESun.value=m*q;d.fKr4PI.value=4*g*Math.PI;d.fKm4PI.value=4*m*Math.PI;g=b.scaleDepth;m=b.innerRadius;q=m+b.height;d.fBias.value=b.bias;d.fExposure.value=b.exposure;d.fScale.value=1/(q-m);d.fScaleDepth.value=g;d.fOuterRadius.value=q;d.fInnerRadius.value=m;d.fOuterRadius2.value=q*q;d.fInnerRadius2.value=m*m}this.stars=[];this.addStar(a.stars||[])}addStar(a){Array.isArray(a)||
(a=[a]);for(var b=0;b<a.length;b++)this.stars.push(a[b]),this.uniforms.lightPos.value.push(new h.Vector3),this.uniforms.lightSize.value.push(2*(a[b].radius||0));this.defines.NUM_STARS+=a.length;this.needsUpdate=!0}updateUniforms(){var a=this.uniforms,b=this.stars,d=a.v3LightPos.value,g=a.v3CameraPos.value;g.setFromMatrixPosition(camera.matrixWorld);d.setFromMatrixPosition(light.matrixWorld);this.pbody&&this.pbody.matrixWorld&&(f.copy(this.pbody.matrixWorld).invert(),d.applyMatrix4(f),g.applyMatrix4(f));
d=g.lengthSq();a.fCameraHeight.value=Math.sqrt(d);a.fCameraHeight2.value=d;for(d=0;d<b.length;d+=1)b[d].getWorldPosition(a.lightPos.value[d]),a.lightPos.value[d].applyMatrix4(f)}addUniforms(a){super.addUniforms(a);a.fg={type:"f",value:0};a.fg2={type:"f",value:0};a.fKrESun={type:"f",value:0};a.fKmESun={type:"f",value:0};a.fKr4PI={type:"f",value:0};a.fKm4PI={type:"f",value:0};a.fScale={type:"f",value:0};a.fBias={type:"f",value:0};a.fExposure={type:"f",value:0};a.fScaleDepth={type:"f",value:0};a.fOuterRadius=
...
(function(h,e){class f extends e.CustomMeshPhysicalMaterial{constructor(c){super(c);this.setDefines({NUM_ECLIPSERS:0});c.planetRadius&&(this.uniforms.planetRadius.value=c.planetRadius);c.eclipsers&&(c=c.eclipsers,Array.isArray(c)||(c=[c]),this.defines.NUM_ECLIPSERS=c.length)}addUniforms(c){super.addUniforms(c);c.planetRadius={type:"f",value:0}}getVertexChunks(){var c=super.getVertexChunks();c.unshift({after:/<common>/,shader:["  // The planetary radius","  uniform float planetRadius;","  // Pass world space to fragments",
"  varying vec3 vertexPos;","  varying vec3 originPos;"]});c.unshift({after:/<begin_vertex>/,shader:["  // convert from local space to camera space","  vertexPos = vec3(modelViewMatrix * vec4(position, 1.0));","  originPos = vec3(modelViewMatrix * vec4(0.0, 0.0, 0.0, 1.0));"]});return c}getFragmentChunks(){var c=super.getFragmentChunks();c.unshift({after:/<common>/,shader:["  // The planetary radius","  uniform float planetRadius;","  // Pass world space to fragments","  varying vec3 vertexPos;",
